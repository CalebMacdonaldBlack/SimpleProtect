...
public class Events implements Listener {
	...
	public void blockDestroy(BlockBreakEvent e) {
		...
		if (e.getBlock().getType().equals(StaticVariables.getMaterial())
			...
			//check if player owns the land
			if(WorldGuardPlugin.inst().getRegionManager(e.getBlock().getWorld()).getApplicableRegions(e.getBlock().getLocation()).isOwnerOfAll(WorldGuardPlugin.inst().wrapPlayer(e.getPlayer())))
			// initiate delete claim method.
			RegionHandling.deleteClaimedLand(e);
			else{
			e.setCancelled(true);
			e.getPlayer().sendMessage(ChatColor.RED + "You cannot unclaim land that does not belong to you!");
			}
